      ...
      <div className="w-full h-[90vh] grid grid-cols-9 grid-rows-7 gap-1 relative">
          ...
          return (
            ...
            <span className="text-lg text-center font-semibold">
              {rules[customIndex]}
                ...
                {customIndex === 59 && (
                    <div
                        className="flex items-center justify-center font-bold text-6xl border-black border-8 rounded-full p-2 w-24 h-24">
        ...
        <div className="flex items-center justify-center">
          <div className={`dice ${rolling ? 'rolling' : ''}`} id="dice">
            <div className={`dot one ${showDots().includes('one') ? 'visible' : ''}`}/>
            <div className={`dot two ${showDots().includes('two') ? 'visible' : ''}`}/>
            <div className={`dot three ${showDots().includes('three') ? 'visible' : ''}`}/>
            <div className={`dot four ${showDots().includes('four') ? 'visible' : ''}`}/>
            <div className={`dot five ${showDots().includes('five') ? 'visible' : ''}`}/>
            <div className={`dot six ${showDots().includes('six') ? 'visible' : ''}`}/>
            <div className={`dot seven ${showDots().includes('seven') ? 'visible' : ''}`}/>
          </div>
          <div
